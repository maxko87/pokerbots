...
public class AdvancedBrain extends GenericBrain{
	...
	private final float[] MIN_WIN_TO_RAISE = new float[] {.8f, .8f, .75f, .75f};
	...
	public String takeAction(OpponentStats o, GetActionObject g, float w, int s) {
		...
		if ((weBeatThem() && state.equals("bet_state")) || weBluffThem() || (playAnywaysFlag && playAnyways())){
	...
	private boolean weBeatThem() {
		...
		if (street == 1 && winChance > defaultOppChance )
			state = "bet_state";
		...
		for ( int i = getActionObject.lastActions.length-1; i >= 0; i-- ) {
			...
			else if ( performedAction.actionType.equalsIgnoreCase("bet") && performedAction.actor.equalsIgnoreCase(this.game.oppName)  ) {
			...
			else if ( performedAction.actionType.equalsIgnoreCase("raise") && performedAction.actor.equalsIgnoreCase(this.game.oppName)  ) {
